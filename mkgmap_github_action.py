...
def main():
    ...
    if 'cached-downloads' in sys.argv:
        with open('downloads.json', 'r') as d:
            downloads = json.load(d)
    else:
        downloads = Downloads(regions).downloads
    ...
    with open(".github/workflows/mkgmap.yml", "w", encoding="utf8") as workflow:
        workflow.write(template.render(downloads=downloads, regions=regions))
